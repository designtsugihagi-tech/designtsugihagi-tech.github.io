...
const splitTextElements = () => {
    const split = (el, cls) => {
        // aタグやspanタグがある場合は、その中身のテキストノードを探して分割
        const children = Array.from(el.childNodes);
        
        // 既に処理済みでないか確認しつつ、テキストノードを分割
        // 再帰処理ではなく、特定のクラスを持つspan（章番号など）の中身を対象にする
        const targets = el.querySelectorAll('.toc-num, .toc-title')
        if (targets.length > 0) {
